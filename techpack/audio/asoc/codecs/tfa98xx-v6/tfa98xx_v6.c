...
static enum Tfa98xx_Error tfa9874_calibrate(struct tfa98xx *tfa98xx, int *speakerImpedance)
{
	...
	err = tfa_dsp_cmd_id_write_read_v6(tfa98xx->tfa, MODULE_FRAMEWORK, FW_PAR_ID_GET_STATUS_CHANGE, 3, (unsigned char *)buffer);
...
bool is_tfa98xx_series_v6(int rev){
    bool ret = false;
    if((rev == 0x74) || (rev == 0x80) || (rev == 0x81) || (rev == 0x94)) {
        ret = true;
    }
    return ret;
}
